"""Phase 4 -- build an order object and show exactly what it would do.

**Nothing in this module submits anything.** build_option_order constructs the
SDK's order object and returns it; simulate_order prints a preview. Submission
is Phase 5 and does not exist yet.

Deliberately, this module does not import, reference, or wrap the SDK's
submission call. Not behind a flag, not commented out. A build step that
*could* submit under some condition is not a build step.
...
multiplies the cash by 100 per contract. Everything else in the preview is
