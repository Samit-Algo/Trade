"""Phase 6 -- what you hold, and what it is actually worth.

Read-only. Nothing here closes a position, builds an order, or references
submission. It reports.

Two decisions govern this module, and both are about honesty.

**Positions are valued at the BID.** A position is worth what someone will
pay for it, not what someone is asking for it and not what it last traded at.
Tiger's own `unrealized_pnl` is computed from `market_price`, which the docs
say maps to `latestPrice` -- a historical fact about a trade someone else
made, possibly hours ago on a thin contract. Valuing at the ask or the last
price flatters every position by the width of the spread, which on options is
not a rounding error.

**The expiry warning is about cash, not about time.** Time decay is the
obvious risk and it is the smaller one. The larger one is that an
in-the-money option left to expire is automatically exercised into a share
transaction, and the cash that transaction needs has nothing to do with what
the option cost. One AAPL 360 call costing $28 exercises into $36,000 of
stock. That is the number the warning prints.

Documented call used, read-only:
...
class PositionError(Exception):
    """Positions could not be read or understood."""
